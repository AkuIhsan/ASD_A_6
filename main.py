...
def Tampilan_Menu() :
    ...
    print("TuDo adalah aplikasi berbasi CLI (Command Line Interface) yang bertujuan membantu pengguna khususnya di bidang manajemen dalam mengatur tumpukan dokumen")
    console.print("[bold magenta]Menu untuk mengatur tumpukan[/bold magenta]")
    ...
    console.print("[bold magenta]Menu untuk mengatur dokumen[/bold magenta]")
    console.print("5. Ambil dokumen dari Tumpukan")
    console.print("6. Buat dan taruh dokumen baru ke antrian")
    console.print("7. Mencari dokumen di tumpukan\n")
    console.print("[bold magenta]Menu untuk mengatur antrian[/bold magenta]")
    console.print("8. Membuat antrian")
    console.print("9. Melihat isi antrian")
    console.print("10. Menerima antrian")
    console.print("11. Menolak antrian")
    console.print("12. Menghapus Antrian\n")
    console.print("[bold magenta]Keluar Program[/bold magenta]")
    console.print("13. Keluar")
...
def baca_data_tumpukan() :
    ...
    # Cek apakah dile tumpukan.json ada
...
def simpan_perubahan_tumpukan() :
    """ Fungsi untuk menghubungkan perubahan antar file tumpukan.json dengan Stack"""
    # Cek apakah dile tumpukan.json ada
    if not Cek_Keberadaan_File(PathTumpukanExist) :
        return
...
def hapus_tumpukan() :
    """ Fungsi untuk menghapus tumpukan"""
    
    # Cek apakah file tumpukan.json
    if not Cek_Keberadaan_File(PathTumpukanExist) :
        return "Tidak ada tumpukan" 
...
def edit_tumpukan() :
    """ Fungsi untuk mengedit tumpukan """
    # Cek apakah dile tumpukan.json ada
    if not Cek_Keberadaan_File(PathTumpukanExist) :
        return "[bold red]Tidak ada tumpukan[bold red]"
    
    ukuran = str(console.input("Masukkan ukuran : "))
    
    # Validasi input
    if ukuran.isnumeric() :
        konfirmasi = console.input("[bold red]Apakah anda ingin melakukan perubahan ? [/bold red](ya/tidak) : ")
        if konfirmasi == "ya" :
            tumpukan.SetSize(int(ukuran))
            return "[bold red]Perubahan berhasil dibuat[bold red]"
                
        elif konfirmasi == "tidak" :
            return "[bold red]Anda membatalkan perubahan [bold red]"

        else :
            return "[bold red]Anda tidak memilih dengan pilihan yang ada[bold red]"

    else :
        return "[bold red]Anda tidak memasukkan angka[bold red]"
...
def ambil_dokumen() :
    """ Fungsi untuk mengambil dokumen"""

    # Validasi File
    if not Cek_Keberadaan_File(PathTumpukanExist) or tumpukan.isEmpty():
        return Rule("[bold red]Tidak ada tumpukan/Tumpukan dalam kondisi kosong[/bold red]")
...
def buat_taruh_dokumen() :
    """ Fungsi untuk membuat dan menaruh dokumen ke dalam antrian"""
    # Validasi file
    if not Cek_Keberadaan_File(PathAntrianExist) :
    ...
    return "Dokumen berhasil dibuat dan dimasukkan ke antrian"

###################################################################################################
#                                   Operasi untuk Antrian                                         #
###################################################################################################
def baca_antrian() :
    """ Fungsi Untuk Membaca data dari JSON untuk data antrian"""
    # validasi file
    if not Cek_Keberadaan_File(PathAntrianExist) :
        return
    
    with open(PathAntrian, 'r') as file:
        antrian.reset()
        data = json.load(file)
        for item in data :
            antrian.enqueue(item)

def simpan_perubahan_antrian() :
    """ Fungsi untuk menyimpan perubahan antar antrian.json dengan queue.py"""
    # validasi file
    if not Cek_Keberadaan_File(PathAntrianExist):
        return
    
    antrian_list = list(antrian.data)
    json_str = json.dumps(antrian_list, indent=4)
    with open(PathAntrian, "w") as f:
        f.write(json_str)
...
def buat_antrian() :
    ...
    if Cek_Keberadaan_File(PathAntrianExist) : 
        return "Antrian Sudah Ada "

    with open(PathAntrian, "w") as f:
        json.dump([], f)

    return "Antrian Berhasil Dibuat!"
...
def lihat_antrian() :
    ...
    # validasi file
    if not Cek_Keberadaan_File(PathAntrianExist) :
        return Rule("[bold red] Tidak Ada Antrian [/bold red]")
...
def terima_antrian() :
    """ Fungsi untuk memindahkan dokumen dari antrian ke tumpukan"""
    # Validasi input
    if not Cek_Keberadaan_File(PathTumpukanExist) :
        return "Tidak ada tumpukan"
    ...
    if tumpukan.isFull() :
        return "Tumpukan sudah penuh"
    
    data = antrian.dequeue()
    tumpukan.push(data)
    return "Dokumen berhasil diterima"

def tolak_antrian() :
    """ Fungsi untuk menolak memindahkan dokumen dari antiran ke tumpukan"""
    # Validasi input
    if not Cek_Keberadaan_File(PathAntrianExist) :
        return "Tidak ada antrian"
    ...
    antrian.dequeue()
    return "Dokumen Berhasil ditolak"

def hapus_antrian() :
    remove(PathAntrian)
    return "Antrian Berhasil Dihapus"
...
def main() :
    ...
    while lanjut :
        ...
        if pilihan == "1" :
            ...
            console.print(Rule(f"[bold red]{buat_tumpukan()}[/bold red]"))

        elif pilihan == "2" :
            system("cls" if name == "nt" else "clear")
            if not lihat_tumpukan() :
                console.print(Rule(f"[bold red] Tidak ada tumpukan [/bold red]"))
            else :
                console.print(Rule(f"[bold magenta] Isi Tumpukan [/bold magenta]"))
                console.print(f"Ukuran Tumpukan : {tumpukan.size}")
                console.print(lihat_tumpukan())
            ...
            system("cls" if name == "nt" else "clear")
            console.print(Rule(f"[bold red]{edit_tumpukan()}[/bold red]"))
            simpan_perubahan_tumpukan()

        elif pilihan == "5" :
            system("cls" if name == "nt" else "clear")
            console.print(ambil_dokumen())
            simpan_perubahan_tumpukan()

        elif pilihan == "6" :
            system("cls" if name == "nt" else "clear")
            console.print(Rule(f"[bold red]{buat_taruh_dokumen()}[/bold red]"))
            simpan_perubahan_antrian()
            ...
            system("cls" if name == "nt" else "clear")
            console.print(Rule(f"[bold red]{buat_antrian()}[/bold red]"))

        elif pilihan == "9" :
            system("cls" if name == "nt" else "clear")
            console.print(lihat_antrian())

        elif pilihan == "10" :
            system("cls" if name == "nt" else "clear")
            console.print(Rule(f"[bold red]{terima_antrian()}[/bold red]"))
            simpan_perubahan_tumpukan()
            simpan_perubahan_antrian()
        
        elif pilihan == "11" :
            system("cls" if name == "nt" else "clear")
            console.print(Rule(f"[bold red]{tolak_antrian()}[/bold red]"))
            simpan_perubahan_antrian()
            ...
            if Cek_Keberadaan_File(PathAntrianExist) :
                ...
                if konfirmasi.lower() == "ya" :
                    console.print(Rule(f"[bold red]{hapus_antrian()}[/bold red]"))

                elif konfirmasi.lower() == "tidak" :
                    console.print(Rule(f"[bold red]Antrian gagal dihapus karena tidak disetujui[bold red]"))
                ...
                console.print(Rule(f"[bold red]Tidak ada Antrian[bold red]"))
